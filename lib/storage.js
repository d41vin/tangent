...
export async function removeSessionLink(id, url) {
  const data = await readSessions();
  const existing = data.sessions[id];
  if (!existing) return null;

  const links = (existing.links ?? []).filter((link) => link.url !== url);
  const session = { ...existing, links, updatedAt: Date.now() };
  await storage.set({ sessions: { ...data.sessions, [id]: session } });
  return session;
}
...
export async function importData(payload) {
  ...
  if (!data || typeof data !== 'object') throw new Error('This file is not a valid Tangent backup.');
  if (!('globalNotes' in data) && !('sessions' in data)) {
    throw new Error('This file does not contain any Tangent data.');
  }
